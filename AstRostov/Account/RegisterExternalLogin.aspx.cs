    ...
    public partial class RegisterExternalLogin : System.Web.UI.Page
    {
        ...
        private void ProcessProviderResult()
        {
            ...
            if (!authResult.IsSuccessful)
            {
                ...
                ModelState.AddModelError("Provider", String.Format("Внешняя аутентификация через {0} не удалась.", ProviderDisplayName));

                // To view this error, enable page tracing in web.config (<system.web><trace enabled="true"/></system.web>) and visit ~/Trace.axd
                Trace.Warn("OpenAuth", String.Format("Произошла ошибка при входе через {0})", ProviderDisplayName), authResult.Error);
                return;
            }
